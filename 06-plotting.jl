...
begin
	using Plots # the base plotting package
	using Distributions  
	
	using RDatasets # some famous R datasets
	using LaTeXStrings
...
# Julia Bootcamp - Plotting

There are several great plotting packages for julia. 
...
let
	plot(sin, 0, 2π, label = L"\sin(x)", xlabel = L"x")
	plot!(cos, 0, 2π, label = L"\cos(x)", title = "keywords are merged")
	ylabel!("also like so")
end
...
The attributes work with each of the backends: [http://docs.juliaplots.org/latest/attributes/](http://docs.juliaplots.org/latest/attributes/)

Compatibility of attributes is found in this chart: [https://docs.juliaplots.org/latest/backends/](https://docs.juliaplots.org/latest/backends/)

I find it easiest to use this page to find the right attributes: [https://docs.juliaplots.org/latest/generated/gr/](https://docs.juliaplots.org/latest/generated/gr/). In fact, now we'll go over some cool examples from that page.
"""
...
plot(fd, m = ([:circle :star], [5 4]), marker_z = 10:-1:1, 
	     l = ([:dash :dot], [:black :red], [2 3]),
	     cbar_title = "z value",
	     label = ["first line" "second one"],  # always row-vectors...
	     legend = :topleft
)
...
let
	...
	# you can give a 2-argument function to the marker_z and line_z keyword args:
	# like here, we give the `+` function, so it will map `x + y`
	p2 = scatter(x, y, marker_z = (+), color = :bluesreds, legend = false)
	plot(p1, p2)  # hey! that's two plots! next chapter 👇
end
...
plot(randn(100, 5),   # 5 columns of data
	seriestype = [:line :histogram :scatter :steppre :bar], # 5 types
    leg = false, 
	title = ["($i)" for j in 1:1, i in 1:5], 
	ticks = nothing,
	titleloc = :right, titlefont = font(8),layout = l)
...
let
	...
	plotarray = []  # an empty array of type `Any`

	# now we `push!` plots onto that array:
	push!(plotarray,plot(data[:,1], ticks = nothing, leg = false, title = "(1)"))

	# now the others
	othertypes = [:histogram :scatter :steppre :bar]
	for i in 2:5
		push!(plotarray,plot(data[:,i],
			  seriestype = othertypes[i-1], leg = false, 
			  title = "($i)"))
	end
	# now put together with the layout as before:
	plot(plotarray..., layout = l, titleloc = :right, titlefont = font(8))
end
...
let
    x = -5:0.1:5
    plot(plot(x, (x->begin    # begin-end blocks would allow multi-line bodies
                    y = x ^ 2
                end)), 
		 plot(x, (x->begin
                    sin(x)
...
Sometimes it's useful to make a 3D plot.
"""
...
q(md"""
1. Open VScode and start the julia terminal (type shift-cmd-p for the command palette and start typing `julia`. click on *start REPL*)
2. install the `Plots` package: type `]` and then `add Plots`
3. after it's done, do `backspace` and type `using Plots`
4. write a function `fakelines(n)` that plots as many lines as given via `n`. Use the `Plots.fakedata` function to generate the data.
5. check with `@assert` that the user does not give an `n>10` to avoid a cluttered plot.
""")

# ╔═╡ 2140fd2e-418b-40f7-8ff6-b89b69dc82b9
q(md"""
Upgrade your `fakelines` function such that each line gets a label reading *line i* for line number `i`. Also, give each line a different `markershape`. Look [here](https://docs.juliaplots.org/latest/generated/attributes_series/) for *markershape* for all possible shapes
""")

# ╔═╡ 22ebc855-b9b9-4d59-9eff-38fd2e095882
q(md"""
We want to make a plot with 2 panels, top and bottom. the top panel should show the measured altitude of rudolph, the red-nosed reindeer 🦌 over time, the bottom panel the location of Santa Claus 🎅🏻. Santa is known to travel a lot by air, Rudolph as well (but he likes climbing steep mountains in his free time), hence their altitude measurements are interesting. Here are the details:
* Rudolph's altitude data has been measured at daily frequency from 1st of May 2021 until 24 of december 2021.
* Santa had his altitude tracked every 10 days, starting on the 1st of january 2021, up until 24 of december 2021. 
* After that date, both have disappeared. 🤷
* Both data series look like a random walk 📈, oddly enough. (in julia you can achieve via `cumsum(randn(10))` (for 10 observations, say))
* So, in the top panel, show the movements of Rudolph. 
* Bottom panel, Santa.
* Both should have a red line, no? Also, the line of Santa should be thicker than the one of Rudolph.
* Remove the `yticks` on both plots, such that this looks a bit more like altitude data. 😄
* When you combine the plots, make sure that the x-axis are linked, so we clearly see when the observations for Rudolph started (not before may 1st!)
* look [here](https://docs.julialang.org/en/v1/stdlib/Dates/) for how to construct a date range!
""")
...
manifest_format = "2.0"
project_hash = "de1c109f6122e1461330e01435ede75b8057e712"
